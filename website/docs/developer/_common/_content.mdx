...
The build configurations are defined in the `package.json` file, within the
`xpack` section.
...
utilise the `--config <name>` option.
...
Actions may be defined for the entire project or for a specific build
configuration.

The actions are defined in the `package.json` file, within the
`xpack` section, at the top or inside build configurations.

:::tip

For those who, for various reasons, cannot utilise **xpm**, it is
...
With this extension installed, xpm actions may be very conveniently
...
The build scripts execute on GNU/Linux and macOS. The Windows binaries are
compiled on GNU/Linux, utilising [mingw-w64](https://mingw-w64.org).
...
during the first execution, later executions being able to utilise the cached files.
