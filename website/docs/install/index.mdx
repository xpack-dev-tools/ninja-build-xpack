...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fninja-build?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/ninja-build/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/ninja-build-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fninja-build?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/ninja-build/)

The **xPack Ninja Build** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
portable archives.

## Automated install

The easiest (and recommended) way to install Ninja Build
is by using the **binary xPack**, available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/ninja-build`}><code>@xpack-dev-tools/ninja-build</code></a> from
...
<code>xpacks/@xpack-dev-tools/ninja-build</code>.
...
**ninja-build** package into the project:
...
* identify the platform specific archive for the latest available version
  of Ninja Build, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/ninja-build</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/ninja-build/.content/bin</code>;
* add <code>@xpack-dev-tools/ninja-build</code> to
  `package.json` as a
  development dependency; this associates a specific version of Ninja Build with
...
it is also possible to install Ninja Build only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/ninja-build@latest --global --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/ninja-build --verbose
`} </CodeBlock>
...
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\ninja-build\\xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>.
...
example <code>/Users/ilg/Library/xPacks/ninja-build/xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:
...
* <code>xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-x64.tar.gz</code>
* <code>xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm64.tar.gz</code>
* <code>xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}-darwin-arm.tar.gz</code>
...
example <code>/home/ilg/.local/xPacks/ninja-build/xpack-ninja-build-{customField('upstreamVersion')}-{customField('xpackSubversion')}</code>:
